...
For this variable, I've been going back and forth on whether I should log-transform the search distance (because it can be ran Guassian), or whether to use a log-link function (Poisson Distribution). The poisson glm more closely matches the mean, but the log-transformation is better at capturing the variance. Each step of the way, I have performed each test to try and gauge a best route forward. The poisson glm suggests a significant difference while the log-transformed shows no difference. 
...
raw_plot <- ggplot(sites, aes(x = Location, y = WalkDist)) + 
...
poisson_plot <- ggplot(sites, aes(x = Location, y = WalkDist)) + 
  geom_jitter(color = "grey", width = 0.1) +
  geom_point(aes(x = Location, y = rate, color = Location), size = 2, data = predictions) + 
  geom_errorbar(aes(x = Location, 
                  ymin = rate - SE, 
...
log_plot <- ggplot(sites, aes(x = Location, y = (WalkDist))) + 
  geom_jitter(color = "grey", width = 0.1) +
...
ggarrange(raw_plot, poisson_plot, log_plot, nrow = 1)
```
...
# Log-transformed, Normal
test_webs2 <- augment(webs2, data = sites)
...
raw_plot <- ggplot(sites, aes(x = Location, y = NumWebs)) + 
  geom_jitter(color = "grey", width = 0.1) +
  geom_point(aes(x = Location, y = mean, color = Location), size = 2, data = raw) +
  geom_errorbar(aes(x= Location, 
                    ymin = mean - se, 
...
poisson_plot <- ggplot(sites, aes(x = Location, y = NumWebs)) + 
  geom_jitter(color = "grey", width = 0.1) +
  geom_point(aes(x = Location, y = rate, color = Location), size = 2, data = predictions) + 
  geom_errorbar(aes(x = Location, 
                  ymin = rate - SE, 
  ...
  theme(axis.text.y = element_text(colour = "black", size = 16),
        axis.title.x = element_blank()) +
  theme(legend.position = "none") +
  annotate(geom = "text", x = 1.5, y = 75, label = "***", color = "black", size = 10) +
  ggtitle("Poisson Distribution")
...
log_plot <- ggplot(sites, aes(x = Location, y = (NumWebs))) + 
  geom_jitter(color = "grey", width = 0.1) +
...
raw_plot <- ggplot(sites, aes(x = Location, y = NumSpiders)) + 
  geom_jitter(color = "grey", width = 0.1) +
  geom_point(aes(x = Location, y = mean, color = Location), size = 2, data = raw) +
  geom_errorbar(aes(x= Location, 
                    ymin = mean - se, 
...
poisson_plot <- ggplot(sites, aes(x = Location, y = NumSpiders)) + 
  geom_jitter(color = "grey", width = 0.1) +
  geom_point(aes(x = Location, y = rate, color = Location), size = 2, data = predictions) + 
  geom_errorbar(aes(x = Location, 
                  ymin = rate - SE, 
...
log_plot <- ggplot(sites, aes(x = Location, y = (NumSpiders))) + 
  geom_jitter(color = "grey", width = 0.1) +
  geom_point(aes(x = Location, y = exp(response), color = Location), size = 2, data = predictions) + 
  geom_errorbar(aes(x = Location, 
                  ymin = exp(response - SE), 
...
raw_plot <- ggplot(web_dist, aes(x = Neighbor, y = RetreatDist)) + 
  geom_jitter(color = "grey", width = 0.1) +
...
poisson_plot <- ggplot(web_dist, aes(x = Neighbor, y = RetreatDist)) + 
  geom_jitter(color = "grey", width = 0.1) +
...
log_plot <- ggplot(web_dist, aes(x = Neighbor, y = (RetreatDist))) + 
  geom_jitter(color = "grey", width = 0.1) +
...
ggarrange(raw_plot, poisson_plot, log_plot, nrow = 1, common.legend = TRUE)
```
...
# Urban Forest
global <- glm(round(RetreatDist, 0) ~ tree100m + log_TotalSub + log_tdr + log_tdh + log_patch, data = near, family = poisson)
options(na.action = "na.fail") # Required for dredge to run
model_dredge <- dredge(global, beta = F, evaluate = T, rank = AICc)
options(na.action = "na.omit") # set back to default
...
model.sel(model_dredge) #estimates same signs
#summary(model.avg(model_dredge, subset = delta <= 2))
car::vif(get.models(model_dredge, 1)[[1]]) # all but log_tdh over 2

# This means there is quite a bit of multicollinearity. Let's try removing the variable with the highest vif.
global2 <- update(global, .~. -tree100m)
options(na.action = "na.fail") # Required for dredge to run
model_dredge <- dredge(global2, beta = F, evaluate = T, rank = AICc)
options(na.action = "na.omit") # set back to default
top_model_wd_f <- get.models(model_dredge, subset = 1)[[1]]
out <- summary(top_model_wd_f)
out
...
# Urban Center
global <- glm(round(RetreatDist, 0) ~ log_TotalSub + log_tdr + log_tdh + spec_rad + log_patch + road_length_m, data = near, family = poisson)
options(na.action = "na.fail") # Required for dredge to run
model_dredge <- dredge(global, beta = F, evaluate = T, rank = AICc)
options(na.action = "na.omit") # set back to default
...
model.sel(model_dredge) #estimates same signs
#summary(model.avg(model_dredge, subset = delta <= 2))
car::vif(get.models(model_dredge, 1)[[1]]) #all except tdh above 2
...
# Still pretty high VIF values. Let's remove another

global3 <- update(global2, .~. -road_length_m)
options(na.action = "na.fail") # Required for dredge to run
model_dredge <- dredge(global3, beta = F, evaluate = T, rank = AICc)
options(na.action = "na.omit") # set back to default
top_model_wd_c <- get.models(model_dredge, subset = 1)[[1]]
out <- summary(top_model_wd_c)
out
...
out <- summary(model.avg(model_dredge, subset = delta <= 2))
out
car::vif(get.models(model_dredge, 1)[[1]]) #looks better
```
...
Coefficients:
            Estimate Std. Error z value Pr(>|z|)    
(Intercept) 0.236708   0.223087   1.061    0.289    
log_patch   0.397613   0.009388  42.354  < 2e-16 *** *positive*
log_tdh     0.422293   0.033427  12.633  < 2e-16 *** *positive*
log_tdr     0.066877   0.012460   5.367 7.99e-08 *** *positive*

R-squared = 0.5431401

(conditional average) 
             Estimate Std. Error Adjusted SE z value Pr(>|z|)    
(Intercept)   0.21444    0.22610     0.24357   0.880    0.379    
log_patch     0.40265    0.01246     0.01319  30.529   <2e-16 *** *positive*
log_tdh       0.42115    0.03350     0.03612  11.660   <2e-16 *** *positive*
log_tdr       0.06930    0.01307     0.01405   4.933    8e-07 *** *positive*
log_TotalSub -0.03964    0.02482     0.02684   1.477    0.140     *negative*
...
# Patch Area
nd <- data.frame(log_patch = seq(4.34, 11.78, 0.07), 
                 log_tdr = mean(near$log_tdr),
                 log_tdh = mean(near$log_tdh))
predicted.distance <- augment(top_model_wd_f, 
                              newdata = nd, 
                              se_fit = TRUE)
dist_forest_patch <- ggplot(near, aes(x = log_patch, y = log(RetreatDist))) + 
  geom_point(color = "grey", 
             size = 1) +
  geom_line(aes(x = log_patch, y = .fitted), 
            size = 1, 
  ...
  theme(axis.text.x = element_blank(), 
        ...
        axis.text.y = element_blank(),
        axis.title.y = element_blank()) +
  annotate(geom = "text", x = min(predicted.distance$log_patch) + diff(range(predicted.distance$log_patch))/2, y = max(log(near$RetreatDist)), label = "***", color = "black", size = 4)

# Highway Disturbance
nd <- data.frame(log_patch = mean(near$log_patch), 
                 log_tdr = mean(near$log_tdr),
                 log_tdh = seq(2.76, 5.42, 0.025))
predicted.distance <- augment(top_model_wd_f, 
                              newdata = nd, 
                              se_fit = TRUE)
dist_forest_tdh <- ggplot(near, aes(x = log_tdh, y = log(RetreatDist))) + 
  ...
  theme(axis.text.x = element_blank(), 
        ...
        axis.text.y = element_blank(),
        axis.title.y = element_blank()) +
  annotate(geom = "text", x = min((predicted.distance$log_tdh)) + diff(range((predicted.distance$log_tdh)))/2, y = max(log(near$RetreatDist)), label = "***", color = "black", size = 4)
...
predicted.distance <- augment(top_model_wd_f, 
                              newdata = nd, 
                              se_fit = TRUE)
dist_forest_tdr <- ggplot(near, aes(x = log_tdr, y = log(RetreatDist))) + 
  geom_point(color = "grey", 
             size = 1) +
  geom_line(aes(x = log_tdr, y = .fitted), 
            size = 1, 
...
raw_plot <- ggplot(webs, aes(x = Location, y = RetreatHeight)) + 
  geom_jitter(color = "grey", width = 0.1) +
  geom_point(aes(x = Location, y = mean, color = Location), size = 2, data = raw) +
  geom_errorbar(aes(x= Location, 
                    ymin = mean - se, 
...
#saveRDS(bigBoot_height_location, file = "bootstrapping/bigBoot_height_location")
bigBoot_height_location <- readRDS("bootstrapping/bigBoot_height_location")
...
poisson_plot <- ggplot(webs, aes(x = Location, y = RetreatHeight)) + 
  geom_jitter(color = "grey", width = 0.1) +
...
#saveRDS(bigBoot_height_location2, file = "bootstrapping/bigBoot_height_location2")
bigBoot_height_location2 <- readRDS("bootstrapping/bigBoot_height_location2")
...
log_plot <- ggplot(webs, aes(x = Location, y = (RetreatHeight))) + 
  geom_jitter(color = "grey", width = 0.1) +
  geom_point(aes(x = Location, y = exp(response), color = Location), size = 2, data = predictions) +
  geom_errorbar(aes(x= Location, 
                    ymin = exp(response - SE), 
...
Convergence issues that need addressed.
...
global <- glmer(ceiling(RetreatHeight) ~ log_TotalSub + log_tdr + log_tdh + (1 | ID), data = webs, family = poisson)
options(na.action = "na.fail") # Required for dredge to run
model_dredge <- dredge(global, beta = F, evaluate = T, rank = AICc)
options(na.action = "na.omit") # set back to default
...
# Urban Forest
global <- glmer(ceiling(RetreatHeight) ~ tree100m + log_TotalSub + log_tdr + log_tdh + log_patch + (1 | ID), data = webs, family = poisson)
options(na.action = "na.fail") # Required for dredge to run
model_dredge <- dredge(global, beta = F, evaluate = T, rank = AICc)
options(na.action = "na.omit") # set back to default
...
# Urban Center
global <- glmer(ceiling(RetreatHeight) ~ log_TotalSub + log_tdr + log_tdh + spec_rad + log_patch + road_length_m + (1 | ID), data = webs, family = poisson)
options(na.action = "na.fail") # Required for dredge to run
model_dredge <- dredge(global, beta = F, evaluate = T, rank = AICc)
options(na.action = "na.omit") # set back to default
...
Fixed effects:
             Estimate Std. Error z value Pr(>|z|)    
(Intercept)    3.1824     0.6117   5.202 1.97e-07 ***
log_tdr       -0.1846     0.1007  -1.834  0.06665 .  
log_TotalSub   0.4715     0.1633   2.888  0.00388 ** 

R-squared = 0.3635599 (marginal - fixed) 0.9508324 (conditional - fixed and random)

(conditional average) 
             Estimate Std. Error Adjusted SE z value Pr(>|z|)   
(Intercept)    3.3645     1.2653      1.2710   2.647  0.00812 **
log_tdr       -0.1934     0.1006      0.1016   1.903  0.05703 . 
log_TotalSub   0.4611     0.1784      0.1799   2.563  0.01038 * 
log_tdh       -0.3226     0.2678      0.2704   1.193  0.23276 
...
(conditional average) 
              Estimate Std. Error Adjusted SE z value Pr(>|z|)  
(Intercept)   3.479599   1.469543    1.475122   2.359   0.0183 *
log_tdr      -0.213553   0.111308    0.112287   1.902   0.0572 .
log_TotalSub  0.471545   0.218490    0.220246   2.141   0.0323 *
log_tdh      -0.359085   0.274215    0.276807   1.297   0.1945  
tree100m      0.015565   0.008560    0.008623   1.805   0.0711 .
log_patch    -0.087858   0.099543    0.100463   0.875   0.3818
...
(conditional average) 
               Estimate Std. Error Adjusted SE z value Pr(>|z|)  
(Intercept)    3.458641   2.160676    2.174189   1.591   0.1117  
log_tdr       -0.212514   0.112355    0.113366   1.875   0.0609 .
log_TotalSub   0.504709   0.216517    0.218399   2.311   0.0208 *
log_tdh       -0.359085   0.274215    0.276807   1.297   0.1945  
log_patch     -0.087858   0.099543    0.100463   0.875   0.3818  
road_length_m  0.002237   0.004915    0.004962   0.451   0.6521  
spec_rad       0.012420   0.030255    0.030548   0.407   0.6843 
