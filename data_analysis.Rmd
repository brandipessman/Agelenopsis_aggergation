...
options(na.action = "na.fail") # Required for dredge to run
model_dredge <- dredge(global3, beta = "none", evaluate = T, rank = AICc)
options(na.action = "na.omit") # set back to default
top_model_wd_c <- get.models(model_dredge, subset = 1)[[1]]
out <- summary(top_model_wd_c)
out
...
height_tdr <- ggplot(webs, aes(x = log_tdr, y = log(RetreatHeight))) + 
  ...
  theme(axis.text.x = element_blank(), 
        ...
        axis.text.y = element_blank(),
        axis.title.y = element_blank()) +
  annotate(geom = "text", x = min(predictions$log_tdr) + diff(range(predictions$log_tdr))/2, y = max(log(webs$RetreatHeight)), label = ".", color = "black", size = 4)
